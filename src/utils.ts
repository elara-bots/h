...
export function json<T extends object>(obj: T) {
...
export function html(doc: string) {
...
const proxy = `https://emapi.netlify.app/json?url=`;

export async function fetchBin(env: Env, id: string) {
    if (!env.DB_URL || !env.DB_APIKEY) {
        return null;
    }
    const res = await fetch(`${proxy}${encodeURIComponent(`${env.DB_URL}/services/b/v/${id}?key=${env.DB_APIKEY}`)}`).catch(e => e);
...
export async function createBin(env: Env, content: string) {
    if (!env.DB_URL || !env.DB_APIKEY || !env.DB_USER) {
        return null;
    }
    const res = await fetch(`${proxy}${encodeURIComponent(`${env.DB_URL}/services/b?key=${env.DB_APIKEY}`)}`, {
        method: "POST",
        body: JSON.stringify({
            content,
            title: "Elara Hastebin",
            userID: env.DB_USER
        }),
        headers: {
            "content-type": "application/json",
...
const js = (name: string) => `https://cdn.jsdelivr.net/gh/GoldenAngel2/cdn/js/${name}.min.js`;
const css = (name: string) => `https://cdn.jsdelivr.net/gh/GoldenAngel2/cdn/css/${name}.css`;
...
export const generateHTML = () => {
    ...
    <head>
        ...
        <title>Elara Hastebin</title>
        <link href="https://cdn.elara.workers.dev/d/icons/Elara.png" rel="icon" />
        <meta property="og:site_name" content="Elara Services">
        <meta property="og:type" content="website">
        <meta property="og:image" content="https://i.imgur.com/8GJ16GS.gif">
        <meta name="theme-color" content="#00ffe9">
        <meta property="og:description" content="A misc hastebin website.">
        <meta property="og:title" content="Hastebin">
        <meta property="og:url" content="https://h.elara.workers.dev">
        <meta charset="utf-8" />
        ...
        <script type="text/javascript" src="${js("application")}"></script>
        ...
        <div style="display: flex; flex-direction: column; height: 100%; width: 100%;">
            ...
            <div id="key" class="menu" style="display: flex; padding: 16px;">
                ...
                <div id="box2" class="menu-actions">
                    ...
                    <div class="button-wrap" style="position: relative;">
                        <span class="label label--small">ctrl + e</span>
                        <button class="pastebin button-picture function">View On Pastebin</button>
                    </div>
                </div>
            </div>
        </div>
